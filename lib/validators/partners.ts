import { PartnerType, PARTNER_TYPE_VALUES, PARTNER_TYPE_LABELS } from '@/types/auth';
import { z } from 'zod';
...
export const partnerAvailabilitySchema = z
  ...
  .refine(
    ...
    { message: 'Í∞Ä???úÍ∞Ñ?Ä ?ïÎ≥¥???Ä?ÑÏ°¥ ?êÎäî ?¨Î°Ø Ï§??òÎÇò ?¥ÏÉÅ???¨Ìï®?¥Ïïº ?©Îãà??' }
  );
...
export const updatePartnerSchema = z
  ...
  .refine((value: z.infer<typeof updatePartnerSchema>) => Object.keys(value).length > 0, {
    message: '?ÖÎç∞?¥Ìä∏???ÑÎìúÎ•???Í∞ÄÏßÄ ?¥ÏÉÅ ?¨Ìï®?¥Ïïº ?©Îãà??'
  });
