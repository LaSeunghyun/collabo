...
export const issueSessionWithTokens = async ({
  ...
  try {
    ...
    if (!sessionRow) {
      throw new Error('?∏ÏÖò???ùÏÑ±?òÏ? Î™ªÌñà?µÎãà??');
    }
    ...
    if (!refreshRow) {
      throw new Error('Î¶¨ÌîÑ?àÏãú ?†ÌÅ∞???ùÏÑ±?òÏ? Î™ªÌñà?µÎãà??');
    }
...
export const rotateRefreshToken = async (
  ...
  if (!existingRow) {
    throw new Error('?†Ìö®?òÏ? ?äÏ? Î¶¨ÌîÑ?àÏãú ?†ÌÅ∞?ÖÎãà??');
  }
  ...
  if (!sessionRow) {
    throw new Error('ÎßåÎ£å???∏ÏÖò?ÖÎãà??');
  }
  ...
  if (session.absoluteExpiresAt <= current || existing.absoluteExpiresAt <= current) {
    ...
    throw new Error('?∏ÏÖò??ÎßåÎ£å?òÏóà?µÎãà?? ?§Ïãú Î°úÍ∑∏?∏Ìïò?∏Ïöî.');
  }

  if (existing.inactivityExpiresAt <= current) {
    await revokeSessionAndToken(existing.id, session.id, current);
    throw new Error('?•ÏãúÍ∞??úÎèô???ÜÏñ¥ ?∏ÏÖò??ÎßåÎ£å?òÏóà?µÎãà??');
  }
  ...
  const { updatedSession, newRefreshRecord } = await db.transaction(async (tx) => {
    ...
    if (!sessionUpdate) {
      throw new Error('?∏ÏÖò Í∞±Ïã†???§Ìå®?àÏäµ?àÎã§.');
    }
    ...
    if (!refreshInsert) {
      throw new Error('Î¶¨ÌîÑ?àÏãú ?†ÌÅ∞ Í∞±Ïã†???§Ìå®?àÏäµ?àÎã§.');
    }
