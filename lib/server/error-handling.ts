...
export function handleFundingSettlementError(error: unknown): NextResponse {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
        switch (error.code) {
            ...
            case 'P2003':
                return buildApiError('ì°¸ì¡° ë¬´ê²°???¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤.', 400, 'FOREIGN_KEY_CONSTRAINT');
            default:
    ...
    if (error instanceof Prisma.PrismaClientValidationError) {
        return buildApiError('?°ì´??ê²€ì¦??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤.', 400, 'VALIDATION_ERROR');
    }

    if (error instanceof Error) {
        console.error('Unexpected error:', error);
        return buildApiError('?œë²„ ?´ë? ?¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤.', 500, 'INTERNAL_ERROR');
    }
