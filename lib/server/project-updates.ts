...
export class ProjectUpdateNotFoundError extends Error {
  constructor(message = '?ÑÎ°ú?ùÌä∏ ?ÖÎç∞?¥Ìä∏Î•?Ï∞æÏùÑ ???ÜÏäµ?àÎã§.') {
    super(message);
...
export class ProjectUpdateAccessDeniedError extends Error {
  constructor(message = '?ÑÎ°ú?ùÌä∏ ?ÖÎç∞?¥Ìä∏???Ä??Í∂åÌïú???ÜÏäµ?àÎã§.') {
    super(message);
...
const ensureMilestoneBelongsToProject = async (projectId: string, milestoneId: string) => {
  ...
  if (!exists) {
    throw new ProjectUpdateValidationError('ÎßàÏùº?§ÌÜ§??Ï∞æÏùÑ ???ÜÏäµ?àÎã§.');
  }
};
...
export const assertProjectOwner = async (
  ...
  if (!project) {
    throw new ProjectUpdateNotFoundError('?ÑÎ°ú?ùÌä∏Î•?Ï∞æÏùÑ ???ÜÏäµ?àÎã§.');
  }
