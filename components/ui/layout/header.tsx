...
export function Header() {
  ...
  const navigationItems = [
    { href: '/projects', label: '?„ë¡œ?íŠ¸' },
    { href: '/artists', label: '?„í‹°?¤íŠ¸' },
    { href: '/partners', label: '?ŒíŠ¸?? },
    { href: '/community', label: 'ì»¤ë??ˆí‹°' },
    { href: '/announcements', label: 'ê³µì??¬í•­', unreadCount }
  ];

  if (session?.user && canAccessRoute(session.user, '/partners/dashboard')) {
    navigationItems.push({ href: '/partners/dashboard', label: '?ŒíŠ¸???ˆë¸Œ' });
  }

  if (session?.user && canAccessRoute(session.user, '/admin')) {
    navigationItems.push({ href: '/admin', label: 'ê´€ë¦? });
  }
