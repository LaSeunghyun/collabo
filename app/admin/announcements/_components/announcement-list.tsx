...
const formatDate = (value: string | null) =>
  value ? new Date(value).toLocaleString('ko-KR') : 'ë°œí–‰ ?ˆì •';
...
export function AnnouncementList() {
  ...
  const handleDelete = (announcement: AdminAnnouncement) => {
    if (!window.confirm('??ê³µì?ë¥??? œ?˜ì‹œê² ìŠµ?ˆê¹Œ? ?? œ ?„ì—???˜ëŒë¦????†ìŠµ?ˆë‹¤.')) {
      return;
    }
  ...
  if (isError) {
    return (
      <div className="rounded-3xl border border-rose-500/20 bg-rose-500/5 p-10 text-center text-sm text-rose-200">
        ê³µì? ?°ì´?°ë? ë¶ˆëŸ¬?¤ì? ëª»í–ˆ?µë‹ˆ?? ? ì‹œ ???¤ì‹œ ?œë„??ì£¼ì„¸??
      </div>
    );
  }
    ...
    <div className="rounded-3xl border border-white/10 bg-white/[0.02] p-8">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-white/60">?ë‹¨ ê³ ì • ?¬ë??€ ë°œí–‰ ?¼ì •??ì¡°ì •?˜ê³  ?„ìš” ??ê³µì?ë¥??? œ?????ˆìŠµ?ˆë‹¤.</p>
        </div>
