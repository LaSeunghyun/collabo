              ...
              recentDaily.map((day) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-white/70">
                    <span>规巩 {numberFormatter.format(day.visits)}</span>
                    <span>技记 {numberFormatter.format(day.uniqueSessions)}</span>
                    <span>蜡历 {numberFormatter.format(day.uniqueUsers)}</span>
                  </div>
                </li>
              ))
            )}
          ...
          <ul className="mt-4 space-y-2 text-xs text-white/70">
            {recentSignups.length === 0 ? (
              ...
              recentSignups.map((day) => (
                ...
                >
                  <span className="font-semibold text-white">{formatDateLabel(day.date)}</span>
                  <span className="text-white/80">{numberFormatter.format(day.signups)}疙</span>
                </li>
              ))
            )}
          </ul>
