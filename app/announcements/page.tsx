...
const FILTERS = [{ value: 'all', label: '?„ì²´' }, ...ANNOUNCEMENT_CATEGORIES];

const getCategoryLabel = (category: string) =>
  ANNOUNCEMENT_CATEGORY_LABELS[category as AnnouncementCategory] ?? category;

const formatDate = (date: Date | null) =>
  date ? DATE_FORMATTER.format(typeof date === 'string' ? new Date(date) : date) : 'ë°œí–‰ ?ˆì •';
    ...
    <div className="mx-auto max-w-4xl px-4 pb-16">
      <header className="flex flex-col gap-3 py-12">
        <div className="flex items-center gap-2 text-sm text-blue-300/80">
          ...
          {unreadCount > 0 ? <span>?½ì? ?Šì? ê³µì? {unreadCount}ê±?/span> : <span>ëª¨ë“  ?Œì‹???•ì¸?´ë³´?¸ìš”</span>}
        </div>
        <h1 className="text-3xl font-semibold text-white">?Œë«??ê³µì??¬í•­</h1>
        <p className="text-sm text-neutral-300">
          Collaborium ?€??ìµœì‹  ?Œì‹, ?•ì±… ë³€ê²? ?´ë²¤???•ë³´ë¥?ê°€??ë¨¼ì? ?•ì¸?˜ì„¸??
        </p>
      </header>
      ...
      <section>
        {announcements.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-white/10 bg-white/[0.02] p-12 text-center text-sm text-white/60">
            ?„ì§ ê³µê°œ??ê³µì?ê°€ ?†ì–´?? ê³??ˆë¡œ???Œì‹???„í•´?œë¦´ê²Œìš”.
          </div>
        ) : (
