    ...
    <div className="mx-auto max-w-3xl px-4 pb-20">
      <header className="pt-6">
        ...
        <p className="mt-2 text-sm text-white/60">
          {user.name ? `${user.name}?? ` : ''}
          ?„ë¡œ?íŠ¸ ì¶”ì²œ???„í•œ ?¤ë¬¸?¬í•­??ê¸°ë°˜?¼ë¡œ ë¹ ë¥´ê²??„ë¡œ?íŠ¸ë¥?ê¸°íš?´ë³´?¸ìš”. ?µì‹¬ ?•ë³´ ?…ë ¥ê³?ë§¤ë‹ˆ?€ê°€ ê²€? í•©?ˆë‹¤.
        </p>
      </header>
      <section className="mt-8 space-y-4 rounded-3xl border border-white/10 bg-white/5 p-6">
        <p className="text-sm text-white/70">
          ???˜ì´ì§€???„ë¡œ?íŠ¸ ?ì„± ?„ë¡œ?¸ìŠ¤???œì‘?¼ë¡œ, ì¶”í›„ ?¨ê³„?¤ê³¼ ?„ë¡œ?íŠ¸ ë§¤ì¹­ ?Œê³ ë¦¬ì¦˜ê³??°ê²°?©ë‹ˆ??
        </p>
        <div className="rounded-2xl border border-primary/40 bg-primary/10 p-4 text-sm text-primary-foreground/80">
          <p className="font-medium text-primary-foreground">?„ì¬ ê¶Œí•œ</p>
          <p className="mt-1 text-primary-foreground/70">
            ?„ì¬ ??• :{' '}
            <span className="font-semibold">
              {ROLE_LABELS[user.role as keyof typeof ROLE_LABELS]}
            </span>
            . ?„ë¡œ?íŠ¸ ?ì„±, ?˜ì •, ?? œë³´ê¸° ?±ì˜ ê¶Œí•œ??ë¶€?¬ë˜?ˆìŠµ?ˆë‹¤.
          </p>
        </div>
      </section>
    </div>
