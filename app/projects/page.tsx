...
export default function ProjectsPage() {
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      case 'LIVE': return 'ÏßÑÌñâÏ§?;
      case 'SUCCEEDED': return '?±Í≥µ';
      case 'FAILED': return '?§Ìå®';
      case 'DRAFT': return 'Ï§ÄÎπÑÏ§ë';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900">
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-white mb-4">?ÑÎ°ú?ùÌä∏</h1>
          <p className="text-white/60 mb-6">
            ?ÑÌã∞?§Ìä∏?§Ïùò Ï∞ΩÏûë ?ÑÎ°ú?ùÌä∏Î•??òÎü¨Î≥¥Í≥† ?ÑÏõê?¥Î≥¥?∏Ïöî.
          </p>
          ...
          <div className="flex gap-4 mb-6">
            ...
            <div className="flex gap-2">
              {[
                { key: 'all', label: '?ÑÏ≤¥' },
                { key: 'LIVE', label: 'ÏßÑÌñâÏ§? },
                { key: 'SUCCEEDED', label: '?±Í≥µ' },
                { key: 'FAILED', label: '?§Ìå®' }
              ].map(({ key, label }) => (
                <button
                  key={key}
                ...
                >
                  ...
                  <div className="mb-4">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-white/80 text-sm">ÏßÑÌñâÎ•?/span>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <div className="text-white/60">Î™®Í∏à??/div>
                      <div className="text-white font-semibold">
                        {project.currentAmount.toLocaleString()}??
                      </div>
                    </div>
                    <div>
                      <div className="text-white/60">Î™©Ìëú??/div>
                      <div className="text-white font-semibold">
                  ...
                  <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/10">
                    ...
                    <div className="text-white/60 text-sm">
                      {daysLeft > 0 ? `${daysLeft}???®Ïùå` : 'Ï¢ÖÎ£å'}
                    </div>
                  </div>
                </Link>
              );
        ...
        {!loading && projects.length === 0 && (
          <div className="text-center py-12">
            <p className="text-white/60">?ÑÎ°ú?ùÌä∏Í∞Ä ?ÜÏäµ?àÎã§.</p>
          </div>
        )}
      </div>
    </div>
