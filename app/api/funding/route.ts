...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return buildError('?”ì²­ ë³¸ë¬¸???•ì¸?????†ìŠµ?ˆë‹¤.');
  }
  ...
  if (!projectId) {
    return buildError('?„ë¡œ?íŠ¸ ?•ë³´ê°€ ?„ë½?˜ì—ˆ?µë‹ˆ??');
  }

  const project = await prisma.project.findUnique({ where: { id: projectId } });
  if (!project) {
    return buildError('?´ë‹¹ ?„ë¡œ?íŠ¸ë¥?ì°¾ì„ ???†ìŠµ?ˆë‹¤.', 404);
  }

  if (![ProjectStatus.LIVE, ProjectStatus.EXECUTING].includes(project.status as any)) {
    return buildError('?„ì¬ ?íƒœ?ì„œ??ê²°ì œë¥?ì§„í–‰?????†ìŠµ?ˆë‹¤.', 409);
  }
  ...
  try {
    ...
    return buildError(error instanceof Error ? error.message : 'Stripe êµ¬ì„±???˜ëª»?˜ì—ˆ?µë‹ˆ??', 500);
  }
    ...
    try {
      if (paymentIntentId) {
        ...
        if (paymentIntent.status !== 'succeeded') {
          return buildError(`ê²°ì œ ?íƒœê°€ ?„ë£Œ?˜ì? ?Šì•˜?µë‹ˆ?? (?„ì¬ ?íƒœ: ${paymentIntent.status})`, 409);
        }
        ...
        if (!amountReceived) {
          return buildError('ê²°ì œ ê¸ˆì•¡???•ì¸?????†ìŠµ?ˆë‹¤.', 422);
        }
        ...
        try {
          ...
          return NextResponse.json({
            ...
            funding,
            settlement: null,
            warning: '?€?©ì? ?±ê³µ?ˆì?ë§??•ì‚° ?ì„±???¤íŒ¨?ˆìŠµ?ˆë‹¤.'
          });
        }
      }
      ...
      if (checkoutSessionId) {
        ...
        if (session.payment_status !== 'paid') {
          return buildError(`ì²´í¬?„ì›ƒ???„ë£Œ?˜ì? ?Šì•˜?µë‹ˆ?? (?„ì¬ ?íƒœ: ${session.payment_status})`, 409);
        }
      ...
      const message =
        error instanceof Error ? error.message : 'ê²°ì œ ê²€ì¦?ì¤??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤.';
  ...
  if (!normalisedAmount) {
    return buildError('ê²°ì œ ê¸ˆì•¡???¬ë°”ë¥´ì? ?ŠìŠµ?ˆë‹¤.');
  }

  try {
    if (mode === 'checkout') {
      if (!successUrl || !cancelUrl) {
        return buildError('Checkout ?¸ì…˜?ëŠ” ?±ê³µ ë°?ì·¨ì†Œ URL???„ìš”?©ë‹ˆ??');
      }
    ...
    const message = error instanceof Error ? error.message : 'ê²°ì œ ?”ì²­ ì²˜ë¦¬???¤íŒ¨?ˆìŠµ?ˆë‹¤.';
    return buildError(message, 500);
  }
}
