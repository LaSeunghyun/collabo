...
export async function GET(request: NextRequest) {
  try {
    // ?∞Ïù¥?∞Î≤†?¥Ïä§ ?¨Ïö© Í∞Ä???¨Î? ?ïÏù∏
    if (!isDrizzleAvailable()) {
      return NextResponse.json(
        { 
          error: '?∞Ïù¥?∞Î≤†?¥Ïä§???∞Í≤∞?????ÜÏäµ?àÎã§.',
          details: 'DATABASE_URL???§Ï†ï?òÏ? ?äÏïò?µÎãà??'
        },
        { status: 503 }
      );
    }
    ...
    // const cursor = searchParams.get('cursor'); // TODO: DrizzleÎ°??ÑÌôò ?ÑÏöî
    ...
    // TODO: DrizzleÎ°??ÑÌôò ?ÑÏöî
    const posts: PostWithAuthor[] = [];

    // const popularityPoolSize = Math.max(limit * 3, FEED_CONFIG.popularLimit * 3, FEED_CONFIG.trendingLimit * 3); // TODO: DrizzleÎ°??ÑÌôò ?ÑÏöî
    ...
    const response: CommunityFeedResponse = {
      ...
      meta: {
        nextCursor,
        total: 0, // TODO: DrizzleÎ°??ÑÌôò ?ÑÏöî
    ...
    console.error('Ïª§Î??àÌã∞ ?ºÎìú Ï°∞Ìöå Ï§??§Î•ò Î∞úÏÉù:', {
      error: error instanceof Error ? error.message : String(error),
      stack: error instanceof Error ? error.stack : undefined,
    ...
    return NextResponse.json(
      { 
        error: 'Ïª§Î??àÌã∞ ?ºÎìúÎ•?Î∂àÎü¨?§Îäî???§Ìå®?àÏäµ?àÎã§.',
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!trimmedTitle || !trimmedContent) {
      ...
      return NextResponse.json({ error: '?úÎ™©Í≥??¥Ïö©?Ä ?ÑÏàò?ÖÎãà??' }, { status: 400 });
    }
