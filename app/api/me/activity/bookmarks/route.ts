...
import { postBookmarks, posts, users, categories } from '@/lib/db/schema/tables';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const bookmarkedPosts = await db
      ...
      .from(postBookmarks)
      .leftJoin(posts, eq(postBookmarks.postId, posts.id))
    ...
    console.error('내 저장글 조회 오류:', error);
    return NextResponse.json(
      { error: '저장한 게시글을 불러오는데 실패했습니다.' },
      { status: 500 }
    );
  }
}
